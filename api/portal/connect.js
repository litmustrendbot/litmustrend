...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const { account_name, strategy, server, account, password, instrument, broker_symbol } = body || {};
        ...
        const selectedInst = instrument || 'XAUUSD';
        const selectedBrokerSymbol = broker_symbol || selectedInst;
        ...
        // Magic number mapping based on strategy tier
        ...
        if (metaApiToken) {
            ...
            if (errMsg.toLowerCase().includes('top up') || errMsg.toLowerCase().includes('reliability') || metaRes.status === 403) {
                return res.status(200).json({
                    ...
                    account: account,
                    server: server,
                    strategy: strategy || 'The game — Risk Taker (10% Risk)',
                    instrument: selectedInst,
                    broker_symbol: selectedBrokerSymbol,
